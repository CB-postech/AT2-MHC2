### this script is for build mouse alveolar merged data
...
so_list_GSE261793 <- list(
    'annotattaed' = readRDS('/home/sjcho/datas/reference_atlas/mouse_lung_alveolar/GSE261793/GSE261793_processed_seurat_annotated.rds'),
    'antibody' = readRDS('/home/sjcho/datas/reference_atlas/mouse_lung_alveolar/GSE261793/GSE261793_processed_seurat_antibody.rds')
)
...
for (subdir in subdir_name) {
    dir_path = paste0('/home/sjcho/datas/reference_atlas/mouse_lung_alveolar/GSE145031/', subdir)
...
tmp = Read10X_h5('/home/sjcho/datas/reference_atlas/mouse_lung_alveolar/GSE204787/GSM6191346_WT_SPCCRE_YFPpos_14dpi.h5')
...
save_path = '/home/sjcho/datas/reference_atlas/mouse_lung_alveolar/outs/test_for_publish_code/'
...
so.merged <- subset(so.raw, nCount_RNA > 1000 & percent.mt < 10)

save_path_mt10 = '/home/sjcho/datas/reference_atlas/mouse_lung_alveolar/outs/test_for_publish_code/mt10/'
...
so.GSE262927 <- subset(so.tmp, cells = Cells(so.tmp)[so.tmp$celltype %in% c('AT1', 'AT1_AT2', 'AT2')])

common.genes = intersect(rownames(so.alv_mt10_del1017), rownames(so.GSE262927))
merged.count <- cbind(so.alv_mt10_del1017@assays$RNA$counts[common.genes,], so.GSE262927@assays$RNA$counts[common.genes,])

so.merged <- CreateSeuratObject(counts = merged.count, project = 'alveolar_merged', min.cells = 0, min.features = 0)
so.merged@meta.data[Cells(so.alv_mt10_del1017), c('sex', 'sample', 'injury_type', 'dpi', 'dataset', 'annotation')] = lapply(so.alv_mt10_del1017@meta.data[, c('gender', 'sample', 'injury_type', 'dpi', 'dataset', 'annotation')], as.character)
