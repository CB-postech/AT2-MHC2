...
save_path = '/home/sjcho/projects/AT2_MHC2/code_for_publish/QC/outs/'
...
so.14.AT2 <- subset(so.alv, cells = Cells(so.alv)[so.alv$dpi == '14dpi' & so.alv$majority_voting == 'AT2'])
so.14.AT2 <- log_normalize(so.14.AT2, save_path, '14dpi_AT2', pcs = 10, nfeatures = 1500)
so.14.AT2 <- AddModuleScore(so.14.AT2, features = list(IL10_genes), name = 'Il10_response')

p <- VlnPlot(so.14.AT2, features = 'Il10_response1', group.by = 'condition', pt.size = 0, cols = c('darkgray', 'darkred')) + geom_boxplot(width = 0.2) + NoLegend()
p <- p + geom_signif(comparisons = list(c('floxed', 'dAT2')), map_signif_level = TRUE)
p <- p + ylim(so.14.AT2$Il10_response1 %>% min, (so.14.AT2$Il10_response1 %>% max * 1.1))
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL10_response.png'), width = 4, height = 3)
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL10_response.pdf'), width = 4, height = 3)
ggplot2pptx(p, 4, 3, paste0(save_path, 'fig4.h.14dpi_AT2_IL10_response.pptx'))

# data from https://www.sciencedirect.com/science/article/pii/S2213671119300840?via%3Dihub
TNFa_IL1b_markers <- read_excel('/home/sjcho/projects/AT2_MHC2/code_for_publish/public_Data/AT2_IL1b_TNFa_treatment_bulkRNAseq.xlsx')
TNFa_IL1b_markers <- data.frame(TNFa_IL1b_markers)
colnames(TNFa_IL1b_markers) <- c('gene', 'IL1b_vs_control_logFC', 'TNFa_vs_control_logFC', 'FDR')
rownames(TNFa_IL1b_markers) = TNFa_IL1b_markers$gene

TNFa_genes = rownames(subset(TNFa_IL1b_markers, FDR < 0.05 & TNFa_vs_control_logFC > log2(2)))
IL1b_genes = rownames(subset(TNFa_IL1b_markers, FDR < 0.05 & IL1b_vs_control_logFC > log2(2)))

so.14.AT2 <- AddModuleScore(so.14.AT2, features = list(TNFa_genes), name = 'TNFa_response')
so.14.AT2 <- AddModuleScore(so.14.AT2, features = list(IL1b_genes), name = 'IL1b_response')

p <- VlnPlot(so.14.AT2, features = 'TNFa_response1', group.by = 'condition', pt.size = 0, cols = c('darkgray', 'darkred')) + geom_boxplot(width = 0.2) + NoLegend()
p <- p + geom_signif(comparisons = list(c('floxed', 'dAT2')), map_signif_level = TRUE)
p <- p + ylim(so.14.AT2$TNFa_response1 %>% min, (so.14.AT2$TNFa_response1 %>% max * 1.1))
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_TNFa_response.png'), width = 3, height = 5)
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_TNFa_response.pdf'), width = 3, height = 5)
ggplot2pptx(p, 3, 5, paste0(save_path, 'fig4.h.14dpi_AT2_TNFa_response.pptx'))

p <- VlnPlot(so.14.AT2, features = 'IL1b_response1', group.by = 'condition', pt.size = 0, cols = c('darkgray', 'darkred')) + geom_boxplot(fill = 'white', width = 0.2) + NoLegend()
p <- p + geom_signif(comparisons = list(c('floxed', 'dAT2')), map_signif_level = TRUE)
p <- p + ylim(so.14.AT2$IL1b_response1 %>% min, (so.14.AT2$IL1b_response1 %>% max * 1.1))
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL1b_response_boxplot.png'), width = 3, height = 5)
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL1b_response_boxplot.pdf'), width = 3, height = 5)
ggplot2pptx(p, 3, 5, paste0(save_path, 'fig4.h.14dpi_AT2_IL1b_response_boxplot.pptx'))
...
so.14.AT2 <- AddModuleScore(so.14.AT2, features = list(subset(h_gene_sets, gs_name == 'HALLMARK_INTERFERON_ALPHA_RESPONSE')$gene_symbol), name = 'HM_IFNa')
so.14.AT2 <- AddModuleScore(so.14.AT2, features = list(subset(h_gene_sets, gs_name == 'HALLMARK_INTERFERON_GAMMA_RESPONSE')$gene_symbol), name = 'HM_IFNg')
so.14.AT2 <- AddModuleScore(so.14.AT2, features = list(subset(C5_gene_sets, gs_name == 'GOBP_RESPONSE_TO_INTERLEUKIN_4')$gene_symbol), name = 'BP_IL4')
so.14.AT2 <- AddModuleScore(so.14.AT2, features = list(subset(C5_gene_sets, gs_name == 'GOBP_RESPONSE_TO_INTERLEUKIN_6')$gene_symbol), name = 'BP_IL6')

p <- VlnPlot(so.14.AT2, features = 'HM_IFNa1', group.by = 'condition', pt.size = 0, cols = c('darkgray', 'darkred')) + geom_boxplot(width = 0.2) + NoLegend()
p <- p + geom_signif(comparisons = list(c('floxed', 'dAT2')), map_signif_level = TRUE)
p <- p + ylim(so.14.AT2$HM_IFNa1 %>% min, (so.14.AT2$HM_IFNa1 %>% max * 1.1))
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IFNa_response.png'), width = 3, height = 5)
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IFNa_response.pdf'), width = 3, height = 5)
ggplot2pptx(p, 3, 5, paste0(save_path, 'fig4.h.14dpi_AT2_IFNa_response.pptx'))

p <- VlnPlot(so.14.AT2, features = 'HM_IFNg1', group.by = 'condition', pt.size = 0, cols = c('darkgray', 'darkred')) + geom_boxplot(width = 0.2) + NoLegend()
p <- p + geom_signif(comparisons = list(c('floxed', 'dAT2')), map_signif_level = TRUE)
p <- p + ylim(so.14.AT2$HM_IFNg1 %>% min, (so.14.AT2$HM_IFNg1 %>% max * 1.1))
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IFNg_response.png'), width = 3, height = 5)
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IFNg_response.pdf'), width = 3, height = 5)
ggplot2pptx(p, 3, 5, paste0(save_path, 'fig4.h.14dpi_AT2_IFNg_response.pptx'))

p <- VlnPlot(so.14.AT2, features = 'BP_IL41', group.by = 'condition', pt.size = 0, cols = c('darkgray', 'darkred')) + geom_boxplot(width = 0.2) + NoLegend()
p <- p + geom_signif(comparisons = list(c('floxed', 'dAT2')), map_signif_level = TRUE)
p <- p + ylim(so.14.AT2$BP_IL41 %>% min, (so.14.AT2$BP_IL41 %>% max * 1.1))
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL4_response.png'), width = 3, height = 5)
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL4_response.pdf'), width = 3, height = 5)
ggplot2pptx(p, 3, 5, paste0(save_path, 'fig4.h.14dpi_AT2_IL4_response.pptx'))

p <- VlnPlot(so.14.AT2, features = 'BP_IL61', group.by = 'condition', pt.size = 0, cols = c('darkgray', 'darkred')) + geom_boxplot(width = 0.2) + NoLegend()
p <- p + geom_signif(comparisons = list(c('floxed', 'dAT2')), map_signif_level = TRUE)
p <- p + ylim(so.14.AT2$BP_IL61 %>% min, (so.14.AT2$BP_IL61 %>% max * 1.1))
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL6_response.png'), width = 3, height = 5)
ggsave(p, file = paste0(save_path, 'fig4.h.14dpi_AT2_IL6_response.pdf'), width = 3, height = 5)
ggplot2pptx(p, 3, 5, paste0(save_path, 'fig4.h.14dpi_AT2_IL6_response.pptx'))
