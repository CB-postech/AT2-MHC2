...
save_path = '/home/sjcho/projects/AT2_MHC2/20241113_after_cellbender/manuscript_figures/figure4_and_exd6_20250225/'
...
p <- DimPlot(so.alv.woInter_woPro, 
        cells.highlight = Cells(so.alv.woInter_woPro)[so.alv.woInter_woPro$majority_voting == 'AT1' & so.alv.woInter_woPro$dpi == '30dpi'], 
        sizes.highlight = 0.5, cols.highlight = "#f66b07") + NoAxes() + NoLegend()
p <- p + ggtitle("")
p <- p + theme(
...
ggsave(p, file = paste0(save_path, '/Dimplot_majority_voting_AT1_30dpi_nontraced.png'), dpi = 1000, width = 3, height = 3.25, bg = "transparent")
ggsave(p, file = paste0(save_path, '/Dimplot_majority_voting_AT1_30dpi_nontraced.pdf'), dpi = 1000, width = 3, height = 3.25, bg = "transparent")
